...
func TestShouldLearnManagedRouteUsesExplicitRouteLearningCandidate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			decision:   RoutingDecision{Valid: true, RouteLearningCandidate: true},
			statusCode: 500,
			want:       true,
		},
		{
		...
		},
		{
			...
			decision:   RoutingDecision{Valid: true, RouteLearningCandidate: false},
			statusCode: 500,
			want:       false,
		},
		{
		...
		},
		{
			...
			decision:   RoutingDecision{Valid: true, RuleID: "committed_stream_failure", RouteLearningCandidate: true},
			statusCode: 200,
			want:       true,
		},
	}
