...
func TestShouldLearnManagedRoutePreservesLegacyBreakerGate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			decision:   RoutingDecision{Valid: true, RouteLearningEligible: true},
			statusCode: 500,
			want:       true,
		},
		{
		...
		},
		{
			...
			decision:   RoutingDecision{Valid: true, RouteLearningEligible: false},
			statusCode: 500,
			want:       false,
		},
		{
		...
		},
		{
			...
			decision:   RoutingDecision{Valid: true, RuleID: "committed_stream_failure", RouteLearningEligible: true},
			statusCode: 200,
			want:       true,
		},
	}
