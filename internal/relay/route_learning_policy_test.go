...
func TestShouldLearnManagedRoutePreservesLegacyBreakerGate(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			decision:   RoutingDecision{Valid: true, RouteLearningEffect: routingRouteLearningCandidate},
			statusCode: 500,
			want:       true,
		},
		{
		...
		},
		{
			...
			decision:   RoutingDecision{Valid: true, RouteLearningEffect: routingRouteLearningNone},
			statusCode: 500,
			want:       false,
		},
		{
		...
		},
		{
			...
			decision:   RoutingDecision{Valid: true, RuleID: "committed_stream_failure", RouteLearningEffect: routingRouteLearningCandidate},
			statusCode: 200,
			want:       true,
		},
	}
