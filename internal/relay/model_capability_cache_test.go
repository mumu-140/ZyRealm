...
func TestModelCapabilityMismatchUsesNegativeCacheWithoutHealthPenalty(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(`{"model":"capability-cache-group","messages":[{"role":"user","content":"hello"}]}`))
	...
	if state := availability.CandidateState(channel.ID, "capability-model", time.Now()); state != availability.StateAvailable {
		t.Fatalf("capability mismatch runtime state = %v, want available", state)
	}
}
